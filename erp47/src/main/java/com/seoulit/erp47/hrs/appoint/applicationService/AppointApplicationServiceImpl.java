...
import com.seoulit.erp47.hrs.appoint.dao.AppointDAO;
...
public class AppointApplicationServiceImpl implements AppointApplicationService {
	...
	public void batchAppoint(List<AppointBean> appointInfoList) {
		for (AppointBean appointBean : appointInfoList) {
			...
			switch (status) {
			case "inserted":
				appointDAO.insertAppointInfo(appointBean);
				break;
			case "deleted":
				appointDAO.deleteAppointInfo(appointBean);
				break;
			case "updated":
				appointDAO.updateAppointInfo(appointBean);
				break;
			}
	...
	public void batchAppointDetail(List<AppointDetailBean> appointDetailInfoList) {
		...
		for (AppointDetailBean appointDetailBean : appointDetailInfoList) {
			...
			switch (status) {
			case "inserted":
				appointDAO.insertAppointDetailInfo(appointDetailBean);
				break;
			case "deleted":
				appointDAO.deleteAppointDetailInfo(appointDetailBean);
				break;
			case "updated":
				appointDAO.updateAppointDetailInfo(appointDetailBean);
				break;
			}
