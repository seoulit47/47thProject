...
import com.seoulit.erp47.hrs.emp.dao.EmployInfoDAO;
import com.seoulit.erp47.hrs.emp.dao.FamilyDAO;
import com.seoulit.erp47.hrs.emp.dao.LicenseDAO;
import com.seoulit.erp47.hrs.emp.dao.MilitaryDAO;
import com.seoulit.erp47.hrs.emp.dao.PerconaldateDAO;
...
public class EmpApplicationServiceImpl implements EmpApplicationService {
	...
	public void batchEmpProcess(Map<String, Object> map) {
		...
		List<EducaBean> educationList = (List<EducaBean>) map.get("educationList");
		List<CareerBean> careerList = (List<CareerBean>) map.get("careerList");
		List<LicenseBean> licenseList = (List<LicenseBean>) map.get("licenseList");
		List<PerconaldateBean> perconaldateList = (List<PerconaldateBean>) map.get("perconaldateList");
		List<MilitaryBean> militaryList = (List<MilitaryBean>) map.get("militaryList");
		List<DisabilityBean> disabilityList = (List<DisabilityBean>) map.get("disabilityList");
	...
	public void batchEmployProcess(Map<String, Object> map) {
		...
		for (BaseBean empBean : employInfoList) {
			...
			if (status.equals("inserted"))
				employinfoDAO.insertEmployInfo(empBean);
			else if (status.equals("updated"))
				employinfoDAO.updateEmployInfo(empBean);
			else if (status.equals("deleted"))
				employinfoDAO.delectEmployInfo(empBean);
		}

		for (BaseBean itverBean : itverList) {
			String status = itverBean.getStatus();
			if (status.equals("inserted"))
				employinfoDAO.insertItver(itverBean);
			else if (status.equals("updated"))
				employinfoDAO.updateItver(itverBean);
			else if (status.equals("deleted"))
				employinfoDAO.delectItver(itverBean);
		}

		for (BaseBean testSubjectBean : testSubjectList) {
			String status = testSubjectBean.getStatus();
			if (status.equals("inserted"))
				employinfoDAO.insertTestSubject(testSubjectBean);
			else if (status.equals("updated"))
				employinfoDAO.updateTestSubject(testSubjectBean);
			else if (status.equals("deleted"))
				employinfoDAO.delectTestSubject(testSubjectBean);
	...
	public void batchJoinCandiProcess(Map<String, Object> map) {
		...
		for (BaseBean joinCandi : joinCandiList) {
			...
			if (status.equals("inserted"))
				employinfoDAO.insertJoinCandi(joinCandi);
			else if (status.equals("updated"))
				employinfoDAO.updateJoinCandi(joinCandi);
			else if (status.equals("deleted"))
				employinfoDAO.delectJoinCandi(joinCandi);
		}

		for (BaseBean joinCandiScore : joinCandiScoreList) {
			String status = joinCandiScore.getStatus();
			if (status.equals("inserted"))
				employinfoDAO.insertJoinCandiScore(joinCandiScore);
			else if (status.equals("updated"))
				employinfoDAO.updateJoinCandiScore(joinCandiScore);
			else if (status.equals("deleted"))
				employinfoDAO.delectJoinCandiScore(joinCandiScore);
		}
	}
	...
	public void batchFinSufcandiProcess(List<FinSufcandiBean> finSufcandiList) {
		for (FinSufcandiBean finSufcandiBean : finSufcandiList) {
			...
			case "inserted":
				employinfoDAO.insertFinSufcandi(finSufcandiBean);
				break;
			case "updated":
				employinfoDAO.updateFinSufcandi(finSufcandiBean);
				break;
			case "deleted":
				employinfoDAO.deleteFinSufcandi(finSufcandiBean);
				break;
